    ...
    CodeBlock.prototype = {
        ...
        run: function() {
            ...
            if (typeof codeStringDict["js"] != "undefined"){
                ...
                this.editor.setValue( addedEditorVal );//#REPLACED to solve duplicate-printing //this.editor.setValue( addedEditorVal + originalEditorVal );                
            }

            //#ORIGINAL
            this.base.add(this.original).trigger("codeblock.run");
            val = this.editor.getValue();
            this.editor.setValue( originalEditorVal ); //#MOD: Added this line.

            //#MOD: Added this chunk. //For Non-JavaScript
            for (var codeType in ["python", "ruby", "java"]){
                languageManager(codeType, codeStringDict);
            }
            function languageManager(languageName, codeStringDict){
                if(typeof codeStringDict[languageName] !="undefined"){ //#QUICK-HACK
                    evalCode(languageName, codeStringDict[languageName].join('\n'), manageConsoleVal, function(msg){console.log(msg)});
                }
            }
            function manageConsoleVal(returnValResult){
                val = returnValResult;
            }
